...
bingo25_li_none = [i for i in range(1, 26)]
bingo_all_li = []
for _ in range(5):
    bingo_order_li = random.sample(bingo25_li_none, 5)
    bingo_all_li.append(bingo_order_li)
    for double_li in bingo_order_li:
        bingo25_li_none.remove(double_li)
    print(" ".join("{:02}".format(num) for num in bingo_order_li))
